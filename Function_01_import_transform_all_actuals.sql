...
CREATE TABLE fpa.fpa_actuals AS (
	SELECT
		...
		CAST(f.subsidiary_no_hierarchy AS CHARACTER VARYING (256)) AS subsidiary,
		CAST(f.department_no_hierarchy_id AS INTEGER) AS department_id,
		CAST(c.department_child_1 AS CHARACTER VARYING (128)) AS department_child_1,
		...
		CAST(f.class_no_hierarchy AS CHARACTER VARYING (128)) AS class_name,
		CAST(f.account_number AS CHARACTER VARYING (128)) AS account_id,
		g.child_gl_account AS child,
		CAST(g.parent_id AS CHARACTER VARYING (4)) AS parent_id,
		g.parent_gl_account AS parent,
		CAST(f.type AS CHARACTER VARYING (128)) AS expense_type,
		CAST(f.account_type AS CHARACTER VARYING (128)) AS account_type,
		CAST(g.fs AS CHARACTER VARYING (13)) AS fs,
		CAST(c.fsli AS CHARACTER VARYING (10)) AS fsli,
		CAST(
			CASE 
			WHEN f.account_number = 1695
		...
		CAST(NULL AS CHARACTER VARYING (27)) AS bucket2,
		...
		CAST(f.custcol_ai_po_line_num AS CHARACTER VARYING(4)) AS po_line_number,
		f.entity AS vendor,
		...
		f.memo,
		CAST(f.item AS CHARACTER VARYING (256)) AS item,
		-- Financial Amounts
		CAST(f.currency AS CHARACTER VARYING (3)) AS currency,
		...
		CAST('Actuals' AS CHARACTER VARYING (128)) AS scenario,
		CAST(NULL AS CHARACTER VARYING (128)) AS team,
		CAST(coup.created_by_fullname AS CHARACTER VARYING (128)) AS contact,
		CAST(coup.created_by_email AS CHARACTER VARYING (128)) AS contact_email,
		CAST(NULL AS CHARACTER VARYING (128)) AS project
	FROM
		fpa.financials f
...
WHERE
	f.accounting_period_end_date <= '2024-05-31' -- Update the month here 
);
